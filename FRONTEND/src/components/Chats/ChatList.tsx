    ...
    return (
        <div className="mx-auto p-6 bg-gray-50 border border-gray-200 rounded-lg shadow">
            ...
            <div className="border border-gray-300 bg-gray-100 p-6 rounded">
                {chats.length === 0 ? (
                    <div>
                        <p className="text-md md:text-lg font-medium">No previous chats</p>
                    </div>
                ) : (
                    <div className="flex flex-col gap-4 ">
                        {chats.map((chat) => (
                            ...
                            >
                                <p className="text-xs md:text-sm text-gray-500 mb-1">{chat.timestamp.toLocaleDateString()}</p>
                                <p className="text-md md:text-lg font-medium">{chat.title}</p>
                            </div>
                        ))}
                    </div>
                )}
